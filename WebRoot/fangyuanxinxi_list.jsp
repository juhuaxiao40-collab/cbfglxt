        ...
        <div class="card-body">
            <div class="pa10 bg-warning">
                <form class="form-inline" id="formSearch" action="?">
                    ...
                    <div class="form-group">
                        房源状态
                        <select class="form-control class_fangyuanzhuangtai" name="fangyuanzhuangtai"
                                id="fangyuanzhuangtai"
                                data-value="${param.fangyuanzhuangtai !=null ? param.fangyuanzhuangtai :''}"
                                style="width:250px;">
                            <option value>请选择房源状态</option>
                            <option value="空闲">空闲</option>
                            <option value="已租">已租</option>
                        </select>
                        <script>$(".class_fangyuanzhuangtai").val($(".class_fangyuanzhuangtai").attr("data-value"))</script>
                ...
                <table width="100%" border="1" class="table">
                    ...
                    <c:set var="i" value="0"/><c:forEach items="${list}" var="map"><c:set var="i" value="${i+1}"/>
                        <tr id="${map.id}" pid="">
                            ...
                            <td>${map.fangyuanmingchen}
                            </td>
                            <td>${map.fangyuanleixing}</td>
                            <td>${map.fangyuanzhuangtai}</td>
                            <td>${map.louceng}
                            </td>
                            <td>${map.fangxing}
                            </td>
                            ...
                            <td align="center">
                                <c:choose>
                                    <c:when test="${  '空闲' == map.fangyuanzhuangtai}">
                                        <a href="zulin_add.do?id=${map.id}" class="btn btn-default">添加租赁</a>
                                    </c:when>
                                </c:choose>

                                <a href="zulin_list.do?fangyuanxinxiid=${map.id}" class="btn btn-default">查询租赁</a>
                                <a href="fangyuanxinxi_detail.do?id=${map.id}" class="btn btn-default">详情</a>
                                <a href="fangyuanxinxi_updt.do?id=${map.id}" class="btn btn-default">编辑</a>
                                <a href="fangyuanxinxidelete.do?id=${map.id}" onclick="return confirm('确定要删除？')"
