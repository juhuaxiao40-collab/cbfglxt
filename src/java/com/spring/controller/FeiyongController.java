...
public class FeiyongController extends BaseController {
    ...
    @Autowired
    private FeiyongService service;
    ...
    public String list() {
        ...
        List<Feiyong> list = service.selectPage(example, page, 12);   // 当前页的记录，设置每页默认12条记录
        ...
        return "feiyong_list";   // 使用视图文件：WebRoot\feiyong_list.jsp
    }
     ...
     *
     */
    @RequestMapping("/feiyong_list_kehu")
    public String listkehu() {
        ...
        setAttribute("sort", sort);      // 把当前排序结果共享数据给前台
        setAttribute("where", where);    // 把当前条件共享数据给前台
        return "feiyong_list_kehu";   // 使用视图文件：WebRoot\feiyong_list_kehu.jsp
    }
...
if (!Request.get("fangyuanleixing").equals("")) {where += " AND fangyuanleixing ='" + Request.get("fangyuanleixing") + "'";}if (!Request.get("kehu").equals("")) {where += " AND kehu LIKE '%" + Request.get("kehu") + "%'"; }if (!Request.get("iszf").equals("")) {where += " AND iszf ='" + Request.get("iszf") + "'";}if (!Request.get("feiyongleixing").equals("")) {where += " AND feiyongleixing ='" + Request.get("feiyongleixing") + "'";}      return where;
    }

    /**
     *  后台添加页面入口
     */
    @RequestMapping("/feiyong_add")
     ...
     * @return
     */
    @RequestMapping("/feiyonginsert")
    public String insert() {
        String tmp="";
        Feiyong post = new Feiyong();  // 创建实体类对象
        // 将前台提交过来的数据依次set到实体类
        ...
        post.setFeiyongleixing(Request.get("feiyongleixing"));
        post.setFeiyongjine(Request.get("feiyongjine"));
        post.setQishiriqi(Request.get("qishiriqi"));
        post.setJiezhiriqi(Request.get("jiezhiriqi"));
     ...
     * 修改数据入口
     */
    @RequestMapping("/feiyongupdate")
    public String update() {
        Feiyong post = new Feiyong();  // 新建实体类
        // 将前台提交过来的数据依次set到实体类
 ...
 if(!Request.get("feiyongleixing").equals(""))         post.setFeiyongleixing(Request.get("feiyongleixing"));
 if(!Request.get("feiyongjine").equals(""))         post.setFeiyongjine(Request.get("feiyongjine"));
 if(!Request.get("qishiriqi").equals(""))         post.setQishiriqi(Request.get("qishiriqi"));
 if(!Request.get("jiezhiriqi").equals(""))         post.setJiezhiriqi(Request.get("jiezhiriqi"));
 ...
 if(!Request.get("tianjiashijian").equals(""))         post.setTianjiashijian(Info.getDateStr());
     ...
     * 后台详情页
     */
   @RequestMapping("/feiyong_detail")
    public String detail(){
        int id = Request.getInt("id");
        Feiyong map = service.find(id);  // url 参数中的id获取行数据
        setAttribute("map" , map);  // 把数据共享给前台
        return "feiyong_detail";  // 详情页面：WebRoot\feiyong_detail.jsp
    }
   ...
   @RequestMapping("/feiyongdelete")
    public String delete(){
        ...
        Feiyong map =  service.find(id);        service.delete(id);// 根据id 删除某行数据
